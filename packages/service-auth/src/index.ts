 ...
 *
 * Provides JWT-like authentication for inter-service communication
 * using Ed25519 signatures instead of symmetric secrets.
 *
 * Design:
 * - Each service has its own Ed25519 keypair
 * - Tokens include: serviceId, nodeId, timestamp, expiry, signature
 * - Verification uses public key registry (no shared secrets)
...
export interface ServiceKey {
  ...
  nodeId: string
  publicKey: string
  privateKey?: string // Only present for local service
}

/**
 * Create a canonical message for token signing
 */
 ...
 * @param privateKey - Ed25519 private key (hex with or without 'ed25519_' prefix)
...
export async function generateToken(
  ...
  // Sign with Ed25519
  const cleanKey = privateKey.startsWith('ed25519_')
  ...
  const signature = `ed25519_${Buffer.from(signatureBytes).toString('hex')}`
...
export async function verifyToken(
  ...
  try {
    ...
    const cleanSignature = token.signature.startsWith('ed25519_')
      ? token.signature.substring(8)
      : token.signature

    const cleanPublicKey = publicKey.startsWith('ed25519_')
 ...
 *
 * This allows services to verify tokens from other services
 * without needing a database query on every request.
 */
